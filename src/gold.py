...
src/gold.py
-----------
Camada Gold — dados agregados e analíticos prontos para consumo.

Responsabilidade: receber o DataFrame Silver e persistir nos dois
destinos de armazenamento:

  1. MongoDB Atlas (NoSQL documental)
       Coleções espelhando o load.py original, mantendo compatibilidade
       com o mcp_server.py e o chatbot Streamlit.

  2. SQLite (relacional / analítico)
       Tabela única `indicadores_pnad` para consultas SQL ad-hoc,
       dashboards e ferramentas de BI que preferem SQL padrão.

Justificativa arquitetural — por que dois paradigmas?
  • MongoDB é ideal para consultas flexíveis por documento (filtro por
    UF + sexo + período) e para servir a API do chatbot via MCP.
  • SQLite (ou PostgreSQL em produção) é ideal para agregações analíticas
    (GROUP BY, window functions, JOINs) e integração com ferramentas de BI
    como Metabase, Power BI ou dbt.
  A combinação evita que um único banco precise ser ótimo nos dois casos.
"""
...
# Mapeamento indicador → coleções MongoDB (mesma convenção do load.py original)
_MONGO_COLECOES = {
    "desocupacao": {
...
class Gold:
    """Persiste o DataFrame Silver nas camadas Gold (MongoDB + SQLite)."""
    ...
    def save(self, df_silver: DataFrame) -> None:
        """Persiste o DataFrame nos dois destinos."""
        rows = [r.asDict() for r in df_silver.collect()]
    ...
    def _save_mongo(self, rows: list[dict]) -> None:
        """
        Grava no MongoDB mantendo a mesma estrutura de coleções do load.py:
          {categoria, periodo, localidade, taxa_<indicador>}
        Estratégia: delete_many + insert_many por coleção (idempotente).
        """
        ...
        try:
            ...
            for row in rows:
                ind   = row["indicador"]
                nivel = row["nivel"]       # N1=Brasil, N3=UF
                key   = f"{ind}|{nivel}"
    ...
    def _save_sqlite(self, rows: list[dict]) -> None:
        """
        Grava em SQLite na tabela `indicadores_pnad`.
        Estratégia: DROP + CREATE + INSERT (idempotente para o escopo do projeto).

        Em produção, substituir por PostgreSQL e usar upsert por chave composta:
          (indicador, localidade, categoria, periodo)
        """
